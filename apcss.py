'''
	Asynchronous Polling Client With Synchronous Server
...
def ClientEntry(port, threadlimit = 1):
	...
	while True:
		if len(work) < threadlimit:
			...
			v = c.sendVectorMessageWithMode4(workitem, mode = IOMode.Async)
			work.append((v, workitem))
		else:
			time.sleep(0.1)
		...
		for workitem in work:
			result = c.handleVectorMessages4(rvector = workitem[0], block = False)
			if result is None:
				continue
			# result[0] - server vector
			# result[1] - reply vector (reply to our vector)
			# result[2] - message
			print('finish', result[2])
			tr.append(workitem)
		for t in tr:
			work.remove(t)
...
def main():
	print('Asynchronous Polling Client With Synchronous Server')
